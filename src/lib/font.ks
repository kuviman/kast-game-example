...
const js_obj = [T] @opaque_type;
const js_obj_iter = [T] (obj :: js_obj[T]) -> std.iter.Iterable[type (String, T)] => (
    .iter = f => (
        (@native "async({obj,f})=>{for(const [key,value]of Object.entries(obj)){await f({0:key,1:value})}}")(
            .obj,
            .f,
        );
    ),
...
const js_list = [T] @opaque_type;
const js_list_iter = [T] (list :: js_list[T]) -> std.iter.Iterable[T] => (
    .iter = f => (
        (@native "async({list,f})=>{for(const x of list){await f(x)}}")(
            .list,
            .f,
        );
    ),
...
const Config = newtype (
    ...
    .kerning :: Float32,
    .space_size :: Float32,
    .chars :: js_obj[TilePos],
    .ranges :: js_list[RangeConfig],
);
...
impl Font as module = (
    ...
    const load = (path :: String) -> Font => (
        let config :: Config = fetch_string(path + "/config.json") |> json_parse;
        ...
        for (c, tile_pos) in config.chars |> js_obj_iter do (
            add_c(c |> single_char, tile_pos);
        );
        for range in config.ranges |> js_list_iter do (
